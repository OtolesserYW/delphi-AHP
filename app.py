...
# 🌟 新增：将名次转换为标准AHP标度值的算法
def rank_to_slider(rank_i: int, rank_j: int, n: int) -> str:
    """根据两个指标的名次差异，智能分配两两比较的权重标度"""
    weight_i = n - rank_i # 名次越靠前(如第1名, 索引0)，分配的权重分越高
    weight_j = n - rank_j
    ...
    if weight_i > weight_j:
        ...
        k = int(ratio + 0.5) # 四舍五入取整
        k = min(k, 9)        # 最高限制到 9
        if k <= 1: return "1 (同等重要)"
        for opt in SLIDER_OPTIONS:
            if opt.startswith(f"左{k}"): return opt
    else:
        ratio = weight_j / weight_i
        k = int(ratio + 0.5)
        k = min(k, 9)
        if k <= 1: return "1 (同等重要)"
        for opt in SLIDER_OPTIONS:
            if opt.startswith(f"右{k}"): return opt
...
L2_A_DEFS = [
    "A1.1 病原体生物危害分级，A1.2 病原谱构成，A1.3 病原体载量。",
...
def matrix_input(matrix_key: str, parent_title: str, items: list, defs: list = None):
    ...
    if defs:
        with st.expander("📖 点击查看指标含义说明 (及包含的下级指标)"):
            for item_label, definition in zip(items, defs):
                st.markdown(f"**{item_label}**：{definition}")

    n = len(items)
    
    # 🌟 新增：智能排序与自动填表外挂（仅当指标数量 >= 3 时显示）
    if n >= 3:
        with st.expander("💡 【辅助工具】太繁琐或易冲突？点此排个序，系统自动帮您打分 👇"):
            st.markdown(
                "<p style='color:#0f766e; font-size:16px; margin-bottom:10px;'>"
                "系统将根据您的排名顺序，自动在下方推算出一套<strong>完全符合严密逻辑（CR ≈ 0）</strong>的权重标度。生成后您仍可手动微调。"
                "</p>", unsafe_allow_html=True
            )
            
            ranked_items = st.multiselect(
                "请按重要程度从高到低依次选取指标（第一选最重要，最后选最次要）：",
                options=items,
                key=f"rank_{matrix_key}",
                placeholder="请依次点击下方空白处选择..."
            )
            
            # 当所有指标都被选定排好序后，显示施法按钮
            if len(ranked_items) == n:
                if st.button("✨ 按此排序自动填充下方对比滑块", key=f"btn_{matrix_key}", type="primary"):
                    for i in range(n):
                        for j in range(i + 1, n):
                            r_i = ranked_items.index(items[i])
                            r_j = ranked_items.index(items[j])
                            # 计算出理想的滑块刻度，强制覆盖 session_state
                            recommended_val = rank_to_slider(r_i, r_j, n)
                            st.session_state[f"{matrix_key}_{i}_{j}"] = recommended_val
                    st.rerun() # 刷新页面，让下方的滑块应用新数值
    ...
    for i in range(n):
        for j in range(i + 1, n):
            with st.container(border=True):
                ...
                slider_key = f"{matrix_key}_{i}_{j}"
                choice = st.select_slider(
                    "请滑动选择相对重要程度",
                    options=SLIDER_OPTIONS,
                    value="1 (同等重要)", # 默认值，若 session_state 已有值则会自动覆盖
                    key=slider_key,
    ...
    if n > 1 and all_default:
        ...
        <div style='background-color: #fff4e5; color: #d97706; padding: 18px; border-radius: 8px; border: 1px solid #fde6d8; font-size: 18px; margin-top: 18px;'>
            ⚠️ <strong>一致性提示：</strong> 当前判断矩阵的逻辑冲突偏大 (CR: {cr:.3f})，请微调您的打分使其小于 0.1，<strong>或使用上方的【辅助工具】一键自动纠正</strong>。
        </div>
        """, unsafe_allow_html=True)
        is_valid = False
    else:
...
if not st.session_state["user_authenticated"]:
    ...
    with st.container(border=True):
        st.markdown(f"""
        <h3 style='border-left: none; padding-left: 0; margin-top: 10px !important; text-align: center; color: {TEXT_DARK};'>
            🔒 专家身份验证
        </h3>
        <p style='text-align: center; color: {TEXT_MUTED}; font-size: 17px; line-height: 1.6;'>
            本问卷内部测试中，仅面向受邀专家开放。<br>请输入课题组为您提供的专属问卷访问口令以开启内容：
        </p>
        """, unsafe_allow_html=True)
        ...
        if st.button("验证并进入问卷", type="primary", use_container_width=True):
            if input_pass == USER_PASSCODE:
                ...
                st.error("❌ 口令错误，请核对后重新输入。如需获取口令，请联系课题组负责人。")
