...
<style>
    ...
    div.stButton button p {{
        font-size: 18px !important;    /* 目标字号 */
    }}

    /* ---- 新增：将姓名输入框的标题、内容及占位符统一放大到 18px ---- */
    div[data-testid="stTextInput"] label p {{
        font-size: 18px !important;
        font-weight: 400 !important; /* 保持加粗，使其醒目 */
        color: {TEXT_DARK} !important;
    }}
    div[data-testid="stTextInput"] input {{
        font-size: 18px !important; /* 专家输入的姓名文本字号 */
    }}
    div[data-testid="stTextInput"] input::placeholder {{
        font-size: 18px !important; /* “请输入您的姓名”提示文字字号 */
    }}
...
if not st.session_state["user_authenticated"]:
    ...
    st.stop()  # 关键：未验证通过时直接中断，不渲染下方任何问卷主体
